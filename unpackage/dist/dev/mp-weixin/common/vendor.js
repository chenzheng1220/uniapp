...
function checkDeviceWidth() {
  const { windowWidth: windowWidth2, pixelRatio, platform } = Object.assign({}, wx.getWindowInfo(), {
...
function initRuntimeSocketService() {
  const hosts = "192.168.95.11,127.0.0.1";
  const port = "8090";
  const id = "mp-weixin_EYpmEB";
    ...
    while (src) {
      ...
      if (src) {
        ...
        if (this.options.silent) {
          index.__f__("error", "at node_modules/marked/lib/marked.esm.js:1581", errMsg);
          break;
        } else {
          throw new Error(errMsg);
        }
  ...
  inlineTokens(src, tokens = []) {
    ...
    while (src) {
      ...
      if (src) {
        const errMsg = "Infinite loop on byte: " + src.charCodeAt(0);
        if (this.options.silent) {
          index.__f__("error", "at node_modules/marked/lib/marked.esm.js:1746", errMsg);
          break;
        } else {
          throw new Error(errMsg);
        }
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      switch (token.type) {
        ...
        default: {
          ...
          if (this.options.silent) {
            index.__f__("error", "at node_modules/marked/lib/marked.esm.js:2075", errMsg);
            return "";
          } else {
            throw new Error(errMsg);
          }
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      switch (token.type) {
        ...
        default: {
          const errMsg = 'Token with "' + token.type + '" type was not found.';
          if (this.options.silent) {
            index.__f__("error", "at node_modules/marked/lib/marked.esm.js:2146", errMsg);
            return "";
          } else {
            throw new Error(errMsg);
          }
...
Parser.prototype.parseStyle = function(node) {
  ...
  const list2 = (this.tagStyle[node.name] || "").split(";").concat((attrs.style || "").split(";"));
  const styleObj = {};
  let tmp = "";
